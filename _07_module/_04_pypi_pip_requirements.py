...
pip_commands = [
    ...
    #현재 가상환경에 설치된 패키지 목록을 requirements.txt 파일로 저장
    "python -m pip freeze > requirements.txt",
    # requirements.txt에 적힌 패키지를 한번에 설치
...
def find_missing_packages() -> list[str]:
    ''' requirements.txt 작성된 패키지가 설치되어있는지 확인'''
    missing_packages = [] # 설치안된 패키지를 저장할 list

    for package_name in REQUIRED_PACKAGES:
        try:
            # package = import_module(package_name)
            # 패키지 버전 정보를 문자열로 반환
            # 단, 해당 패키지가 설치안돼있으면 PackageNotFoundError 발생
